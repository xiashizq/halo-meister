...
Support is locked to the latest Xbox/Game Pass (`2026-07-29-wingdk`) and Steam
(`2026-07-25-steam-post-cu2`) `HaloSimulation_tag_release.dll` builds recorded in
the game-build profile catalog. Both builds share the same verified native layout.
...
- object allocator/constructor: module RVA `0x5A0FB0`, returns a 32-bit object datum.
...
- rejected-weapon cleanup: module RVA `0x5A2C20`; a temporary weapon object is
...
- live object model variant: the registered `object_set_variant` script handler at
  ...
  to RVA `0x5A7D00`. Halo Meister calls the latter on the simulation callback after
  validating its prologue, so Customization changes the existing player immediately.
- live per-object colors: RVA `0x5B32C0` receives the controlled object datum, a
  ...
  RVA `0x4586D0` that `object_set_variant` uses to publish the renderer update. The
...
- separate gameplay-cheat hook: the `HaloMeisterCheatInvoke` export reads its own
  ...
  `skull_boots_off_the_ground`). The hook applies the full mask through RVA `0x209FA0`
...
- saved-film open wrapper: module RVA `0x2054E0`; it accepts the same mode-1/full-path
...
Build-specific constants live in `generated_game_build.h`. See
